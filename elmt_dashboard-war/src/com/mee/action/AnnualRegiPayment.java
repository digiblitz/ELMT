...
public class AnnualRegiPayment extends Action {
    
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
            HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        
        HLCVaigaiSessionRemote vaiRemote  = initializeVaigai1EJB(request);
        ...
        if(execute!=null && execute.trim().length()!=0){
            ...
            if(execute.trim().equalsIgnoreCase("listDetails")){
                ...
                ArrayList pendIndo = vaiRemote.getAnnualPendingListDetails(regi_id,pay_id,meet_id);
                
                request.setAttribute("pendInfo",pendIndo);
                return mapping.findForward("listDet");
            }
            ...
            if(execute.trim().equalsIgnoreCase("payProceed")){
                ...
                return mapping.findForward("failPay");
            }
            ...
            if(execute.trim().equalsIgnoreCase("retryPayProceed")){
                ...
                ArrayList det = vaiRemote.getAnnualMeetingDetails(userId,paymentId);
        ...
        // Payment Process
            ...
            if(execute.trim().equalsIgnoreCase("payment")){
                ...
                if(tempCheckAmount!=null && tempCheckAmount.trim().length()!=0){
                 ...
                 if(checkAmount!=0){
     boolean update_refund = remote.updatePendingAmount(sesUserId,payment_id,checkAmount);
                }
                ...
                if(r11.equalsIgnoreCase("check")){
                    ...
                    if(update_stat == true){
                        ...
                        return mapping.findForward("successPay");
                    }
                    else{
                   ...
                   // if(expYear.trim().length()==1) {
                    ...
                    String tmpNova = mr.getMessage("acmRePayment.nova");
    ...
    private HLCVaigaiSessionRemote initializeVaigai1EJB(HttpServletRequest request) throws Exception{
        MessageResources mr=getResources(request);
