 ...
 * @author parteek
 */
public class DonationReports extends Action {
       ...
       HttpSession session = request.getSession();
        ...
        HLCMahanadhiSessionRemote objAccRemote = objAccHome.create();
            ...
            if (process!=null || process.trim().length()> 0) {
                ...
                double totalDonation=objAccRemote.getTotalDonationAmount(startDate, endDate);
                ...
                return mapping.findForward("totalDonationAmount");
                }
                else //For tabular view ...
                {
                    ...
                    return mapping.findForward("totalDonationAmountTabular");
                ...
                ArrayList totalDonationAmountForAllDonationItems=objAccRemote.getTotalDonationAmountForAllDonationItems(startDate,endDate);
                 if(view.equalsIgnoreCase("1"))
                {
                    for(int i=0;i<graph.length;i++)
                    {
                 //Bar Marker Range Chart Generation
                    ...
                    request.setAttribute("graphsCount",graphNew);    
                    return mapping.findForward("totalDonationAmountForAllDonationItems");
                 }
                 else{
                     List list = new ArrayList();
                    ...
                    session.setAttribute("demo", list);
               return mapping.findForward("totalDonationAmountForAllDonationItemsTabular"); 
                 }
                }
                ...
                double donationAmountSpecificDonationItem=objAccRemote.getDonationAmountSpecificDonationItem(startDate,endDate,donation);
                 //PlotFormat Graph
                 if(view.equalsIgnoreCase("1"))
                {
                    for(int i=0;i<graph.length;i++)
