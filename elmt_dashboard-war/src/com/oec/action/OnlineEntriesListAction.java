...
public class OnlineEntriesListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        try{
          ...
          if(process!=null && process.equalsIgnoreCase("initList")){
                ...
                return mapping.findForward("frmProCalUseaStaffList");
                ...
                if(tempYear!=0 && area!=null && area.trim().length()!=0){
                    objProCalList = eveCalRemote.getUseaStaffList(tempYear,area);
                }
                ...
                if(provisionalId!=null && provisionalId.trim().length()!=0){
                    objCalVO = eveCalRemote.getSingleEventDetails(provisionalId);
                }
                request.setAttribute("objCalVO",objCalVO);
                return mapping.findForward("frmProCalUseaStaffView");
                ...
                return mapping.findForward("frmProCalUseaStaffApprove");
                ...
                if(provisionalId!=null && provisionalId.trim().length()!=0 && uStaffStatus!=null && uStaffStatus.trim().length()!=0){
                    result = eveCalRemote.approveEventDetailsByUStaff(provisionalId, uStaffStatus, uStaffComments);
                }
               if(result){
                   ArrayList eveStatList = eveCalRemote.getEventDetsApprovedByAll(provisionalId,eventId);
          ...
          if(orgApprovStat!=null && orgApprovStat.equalsIgnoreCase("Approved") && areaChApprovStat!=null && areaChApprovStat.equalsIgnoreCase("Approved") && uStaffApprovStat!=null && uStaffApprovStat.equalsIgnoreCase("Approved") && oldEventId!=null && eventId!=null){
              boolean output=eveCalRemote.eveDetailsExistsInEveTable(eventId);
              Debug.print("output:" +output);
              if(output==false){
              ArrayList eveStatList1 = eveCalRemote.getOldEventDetails(oldEventId);
              ArrayList eveStatList2 = eveCalRemote.getOldEventHistoryDetails(oldEventId);
           ...
           if(userId!=null && userId.trim().length()!=0){
             ...
             resultStat=eveCalRemote.insertProCalEveIntoEveDetails(objEventDetails);
          ...
          if(eveStatList2!=null && eveStatList2.size()!=0){                  
           ...
           if(eveTitle!=null){
            objEventDetails.setEventTitle(eveTitle);
             ...
             resultHistStat=eveCalRemote.insertProCalEveIntoEveHistoryDetails(objEventDetails);
            ...
            if(resultStat==true){
             provDets=eveCalRemote.getProvEventDetails(eventId);
            }
            if(provDets!=null && provDets.size()!=0){
             ...
             boolean resultStat1=eveCalRemote.updateNewEveDets(orgId,eveTitle,eveBegDt,eveEndDt,oeBegDt,oeEndDt,oeExDueDt,state,area,eventId);
             boolean resultStat2=eveCalRemote.updateOmnibusDetsInAllTables(eventId,oldEventId);
       ...
       boolean output1=eveCalRemote.eveDetailsExistsInEveTable(eventId);
              Debug.print("output1:" +output1);  
       if(output1==false){  
    ArrayList eveList = eveCalRemote.getApprovedEventDet(provisionalId,eventId); 
          ...
          if(orgId!=null){
           ...
           if(StaId!=null){
            ...
            proEveObj.setIssueId(eveIssId);
             ...
             boolean eveResult=eveCalRemote.insertEveDetails(proEveObj);  
             
             
          if(eveResult==true){
              
        ArrayList eveTypLevelsList=eveCalRemote.getEveTyLevIdDet(provisionalId,eventId);
          ...
          if(eveTypLevelsList!=null && eveTypLevelsList.size()!=0){
            ...
            String mapTypeId=eveCalRemote.getEveTypLevelMapId(eveTypeId,eveLevId);
            
           boolean mapResult=eveCalRemote.insertMapDets(eventId,mapTypeId);
            Debug.print("mapResult *****" +mapResult);             
                ...
                if(result){
                    ...
                    return mapping.findForward("EventApproveSuccess");
                }else{
		...
		String area_id = AreaLstArr[0];
                ...
                allAreaList1 = (ArrayList)eveCalRemote.getAllAreaMasters();
                //if(tempYear!=0){
                    objAppEvent = eveCalRemote.getFinalizedEventCalDetails(area);
                //}
		...
		while(itr.hasNext()){
             ...
             if(oldEveId!=null){    
                boolean output=eveCalRemote.eveDetailsExistsInEveTable(newEveId);            
                     if(output==false){
              ArrayList eveStatList1 = eveCalRemote.getOldEventDetails(oldEveId);
              ArrayList eveStatList2 = eveCalRemote.getOldEventHistoryDetails(oldEveId);
              boolean resultStat=false;
              boolean resultHistStat=false;
              if(eveStatList1!=null && eveStatList1.size()!=0){                  
              Iterator itr1 =eveStatList1.iterator();
          ...
          if(eveStatList2!=null && eveStatList2.size()!=0){                  
           ...
           if(eveTitle!=null){
            ...
            ArrayList provDets= new ArrayList();
            if(resultStat==true){
             provDets=eveCalRemote.getProvEventDetails(newEveId);
            }
            if(provDets!=null && provDets.size()!=0){
                String orgId="";
                 String eveTitle="";
             ...
             boolean resultStat1=eveCalRemote.updateNewEveDets(orgId,eveTitle,eveBegDt,eveEndDt,oeBegDt,oeEndDt,oeExDueDt,state,area1,newEveId);
             boolean resultStat2=eveCalRemote.updateOmnibusDetsInAllTables(newEveId,oldEveId);
             Debug.print("resultStat1:" +resultStat1); 
             Debug.print("resultStat2:" +resultStat2);  
       ...
       boolean output1=eveCalRemote.eveDetailsExistsInEveTable(newEveId);
              Debug.print("output1:" +output1);  
       if(output1==false){  
    ArrayList eveList = eveCalRemote.getApprovedEventDet(provisId,newEveId); 
        ...
        ArrayList eveTypLevelsList=eveCalRemote.getEveTyLevIdDet(provisId,eventId);
...
i++;}   
	        ...
	        int currentYear = now.get(Calendar.YEAR);
                ...
                eventLevelDetails=eveCalRemote.getEveLevTypDets(provisionId);
                ...
                if(provisionalId!=null && provisionalId.trim().length()!=0){
                    objCalVO = eveCalRemote.getSingleEventDetails(provisionalId);
                    objEveDets=eveCalRemote.getEveLevTypDets(provisionalId);
                }
                request.setAttribute("objCalVO",objCalVO);
                request.setAttribute("objEveDets",objEveDets);
                return mapping.findForward("frmFinalizedEventCalendarView");
            }
        } catch(Exception e){
            e.printStackTrace();
        }
